...
for ind = 1:length(allAgedMice)
    for run = 1:4
        ...
        if exist(currRunLoc, 'file')
            disp([char(allAgedMice(ind)) '-week0-stim' num2str(run) ' stimdat']);
        ...
        if exist(currRunLoc, 'file')
            disp([char(allAgedMice(ind)) '-week0-stim' num2str(run) ' peakhbdat']);
            currRun = load(currRunLoc);
...
for ind = 1:length(allAgedMiceB)
    for run = 1:4
        ...
        if exist(currRunLoc, 'file')
            disp([char(allAgedMiceB(ind)) '-week0-stim' num2str(run) ' stimdat']);
            currRun = load(currRunLoc);
        ...
        if exist(currRunLoc, 'file')
            disp([char(allAgedMiceB(ind)) '-week0-stim' num2str(run) ' peakhbdat']);
            currRun = load(currRunLoc);
