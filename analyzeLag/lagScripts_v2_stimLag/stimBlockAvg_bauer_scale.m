...
%% input parameters
ds = '8'; % which mice dataset, 1 or 8
dateDS = '181116'; % which date
useGSR = true;
scale = 'log(10)'; % which scaling

%% load data
disp(['----- LOADING ' dateDS '-' ds '-week0 -----']);
tic;

maskData = load(['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-LandmarksAndMask.mat']);
bauerData1F = load(['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim1-dataFluor.mat']);
bauerData1H = load(['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim1-dataHb.mat']);
bauerData2F = load(['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim2-dataFluor.mat']);
bauerData2H = load(['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim2-dataHb.mat']);
bauerData3F = load(['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim3-dataFluor.mat']);
bauerData3H = load(['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim3-dataHb.mat']);

data4LocF = ['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim4-dataFluor.mat'];
data4LocH = ['D:\ProcessedData\TestData\' dateDS '\scale' scale '\' dateDS '-' ds '-week0-stim4-dataHb.mat'];
...
for ind=1:length(bauerDataF)
    ...
    saveFigLoc = ['D:\ProcessedData\TestData\181116\scale' scale '\stimLagDataBauer\peakHbMapFigs\'...
        dateDS '-' ds '-week0-stim' num2str(ind) '-peakHb_GSR.fig'];
    savePeakDataLoc = ['D:\ProcessedData\TestData\181116\scale' scale '\stimLagDataBauer\peakHbDat\'...
    ...
    if exist(saveFigLoc, 'file')
        ...
        save(savePeakDataLoc, 'peakHbMap');
        close(peakHbMapFig);
    end

    % display peak hb map to base ROI off of
    saveMaskLoc = ['D:\ProcessedData\TestData\181116\scale' scale '\stimLagDataBauer\stimResponseDat\ttraceMasks\'...
    ...
    % save data
    saveDatLoc = ['D:\ProcessedData\TestData\181116\scale' scale '\stimLagDataBauer\stimResponseDat\'...
...
saveRegFig = ['D:\ProcessedData\TestData\181116\scale' scale '\stimLagDataBauer\stimResponseDat\'...
        dateDS '-' ds '-week0-actvRegImg_GSR'];
saveas(actvReg, [saveRegFig '.png']);

close(actvReg);

saveLagFig = ['D:\ProcessedData\TestData\181116\scale' scale '\stimLagDataBauer\stimResponseDat\'...
        dateDS '-' ds '-week0-stim-LagFig_GSR'];
saveas(lagfigStim, [saveLagFig '.png']);

close(lagfigStim);

saveStimFig = ['D:\ProcessedData\TestData\181116\scale' scale '\stimLagDataBauer\stimResponseDat\'...
