function exampleTiff(excelFile,excelRows)
...
%         clear fixedRaw fixedRawTime;
        ...
        representativeCh = runInfo.hbChInd(1);
        qcRawFig = mouse.expSpecific.qcRaw(runInfo,rawTime,raw,representativeCh, droppedFrames);
        ...
        if ~isempty(dfIndRaw)
            msg = 'Issue when removing dropped frames. Dropped frames remain.';
            error(msg);
        end
    end 
    ...
    if ~isempty(runInfo.fluorChInd) % if there are fluor channels
        ...
        [datafluorCorr, datafluor] = fluorProc.process(fluorRaw,datahb,fluorSamplingRate);
