function examplePowerAnalysis(excelFile, rows)
% acquire freq range if specified, if not, use ISA
possibleFreq = {'0.009-0.08Hz (ISA)', '0.08-0.4Hz (Intermediate)', '0.4-4.0Hz (Delta)'};
promptString = 'Please select one or more frequency ranges:';
[indx,tf] = listdlg('ListString', possibleFreq, 'PromptString',promptString, 'ListSize',[250,60]);
fRange = [];
if tf == 1
    for ind=1:numel(indx)
        if indx(ind) == 1
           fRange = [fRange; 0.009 0.08];
        elseif indx(ind) == 2
           fRange = [fRange; 0.08 0.4];
        elseif indx(ind) == 3
            fRange = [fRange; 0.4 4.0];
        end
    end
else
    fRange = [0.009 0.08];
...
for runInfo = runsInfo
    ...
    currentTrialName = [runInfo.recDate, '-', runInfo.mouseName, '-', runInfo.session, num2str(runInfo.run)];
    ...
    if exist(saveFilePowerData, 'file')
        ...
        load(saveFilePowerData, 'mouseRunInfo','speciesList','runFreq','runSpectra', 'runMeanSpectra', 'sessionType');
        ...
        spectraFigure = plotPowerSpec(runFreq, runMeanSpectra, speciesList, currentTrialName);
        mapFigure = plotPowerMap(fRange, runFreq, runSpectra, meanMask, speciesList, currentTrialName);
        ...
        save(saveFilePowerData, 'mouseRunInfo','speciesList','runFreq','runSpectra','runMeanSpectra','sessionType','-v7.3');
        ...
        [numRuns, numSpecies] = size(freqMouse);
            ...
            for ind=1:numSpecies
                for ind2=1:numRuns
                    ...
                    storeMeanSpectraLogSum = storeMeanSpectraLogSum + log(meanSpectraMouse{ind2, ind});
                end
            ...
            mouseAvgSpectraFig = plotPowerSpec(freqMouseAvg, meanSpectraMouseAvg, speciesList, currentTrialName);
            mouseAvgMapFigure = plotPowerMap(fRange, freqMouseAvg, spectraMouseAvg, meanMask, speciesList, currentTrialName);
...
if numMice > 1
    ...
    mouseAvgSpectraFig = plotPowerSpec(freqAllMouseAvg, meanSpectraAllMiceAvg, speciesList, currentTrialName);
    mouseAvgMapFigure = plotPowerMap(fRange, freqAllMouseAvg, spectraAllMiceAvg, meanMask, speciesList, currentTrialName);
    toc % how long it takes to compute and plot avg power
    ...
    for saveInd = 1:numel(mouseAvgMapFigure)
        saveAvgMapFigure = [saveAcrossMousePowerInfo '\' currentTrialName '-powerMapAVG-' fStr{saveInd}];
