function runsInfo = parseDatRuns(excelFile,excelRows)
...
for row = tableRows % for each row of excel file
    ...
    validFolder = false(1,numel(D));
    for dirInd = 1:numel(D)
        validFolder(dirInd) = D(dirInd).isdir ...
            && contains(D(dirInd).name,sessionType) ...
            && contains(D(dirInd).name,['-' mouseName '-']);
    end
    validFolders = D(validFolder);
    
    % find list of runs in this mouse
    runList = nan(1,numel(validFolders));
    for dirInd = 1:numel(validFolders)
        runNumStart = strfind(validFolders(dirInd).name,['-' sessionType]);
        runNumStart = runNumStart + numel(sessionType) + 1;
        
        runNumEnd = find(isstrprop(validFolders(dirInd).name,'digit'));
        runNumEnd(runNumEnd < runNumStart) = [];
        runList(dirInd) = str2double(validFolders(dirInd).name(runNumStart:runNumEnd));
    end
    ...
    for run = uniqueRuns
        ...
        runFolders = validFolders(run == runList);
        
        runFolderList = cell(1,numel(runFolders));
        for dirInd = 1:numel(runFolders)
            runFolderList{dirInd} = fullfile(runFolders(dirInd).folder,runFolders(dirInd).name);
        end
        ...
        runNumStart = strfind(runFolders(1).name,'-');
        saveFileName = runFolders(1).name; saveFileName = saveFileName(1:runNumStart(end-1)-1);
        ...
        runsInfo(totalRunInd).rawFolder = runFolderList;
